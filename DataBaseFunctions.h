...
namespace Gymmanagment {
	...
	public:
		void DeleteEmployee(String^ name)
		{
			SqlConnection^ connection = gcnew SqlConnection("Data Source=tsede;Initial Catalog=Gym;Integrated Security=True;");
			connection->Open();
			SqlCommand^ command = gcnew SqlCommand("exec udpDeleteEmployee @name", connection);
			command->Parameters->AddWithValue("@name", name);
			command->ExecuteScalar();
			connection->Close();
		}
	public:
		void DeleteMember(Object^ name)
		{
			SqlConnection^ connection = gcnew SqlConnection("Data Source=tsede;Initial Catalog=Gym;Integrated Security=True;");
			connection->Open();
			SqlCommand^ command = gcnew SqlCommand("exec udpDeleteMember @name", connection);
			command->Parameters->AddWithValue("@name", name);
			command->ExecuteScalar();
			connection->Close();
		}
...
public:
		void ShowMembers(DataGridView^ dgv)
		{
			connection->Open();
			SqlCommand^ cmd = gcnew SqlCommand("exec udpMemberAttendance",connection);
...
public:
		void ShowMemberAttendance(DataGridView^ dgv)
		{
			connection->Open();
			SqlCommand^ cmd = gcnew SqlCommand("exec udpViewMemberAttendance",connection);
			SqlDataAdapter^ adapter=gcnew SqlDataAdapter(cmd);
			DataTable^ table=gcnew DataTable();
			adapter->Fill(table);
			dgv->DataSource = table;
...
public:
		void ShowEmployeeAttendance(DataGridView^ dgv)
		{
			connection->Open();
			SqlCommand^ cmd = gcnew SqlCommand("exec udpViewEmployeeAttendance",connection);
			SqlDataAdapter^ adapter=gcnew SqlDataAdapter(cmd);
			DataTable^ table=gcnew DataTable();
			adapter->Fill(table);
			dgv->DataSource = table;
...
public:
	void TickMemberAttendance(String^ name)
	{
		connection->Open();
		SqlCommand^ cmd = gcnew SqlCommand("exec udpTickMemberAttendance @name", connection);
		cmd->Parameters->AddWithValue("@name", name);
		cmd->ExecuteNonQuery();
		connection->Close();
	}
public:
	void TickEmployeeAttendance(String^ name)
	{
		connection->Open();
		SqlCommand^ cmd = gcnew SqlCommand("exec udpTickEmployeeAttendance @name", connection);
		cmd->Parameters->AddWithValue("@name", name);
		cmd->ExecuteNonQuery();
		connection->Close();
	}
